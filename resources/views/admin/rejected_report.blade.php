            ...
            <div class="container-fluid">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center mb-5">
                        <h4 class="m-0">REJECTED REPORTS</h4>
                    </div>
                    <div class="card-body">
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#report-table').on('click', '.view-reports', function() {
            ...
            $.ajax({
                type: 'GET',
                url: "{{ url('admin/rejectedreports') }}/" + reportsID + "/show",
        ...
        $('#report-table').on('click', '.delete-reports', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'DELETE',
                        url: '{{ route("rejected_reports.delete", ["report" => "_reportsID_"]) }}'.replace('_reportsID_', reportsID),
                        success: function(response) {
                            Swal.fire(
                                'Report Deleted!',
        ...
        $('#report-table').on('click', '.put-active', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'PATCH',
                        url: '{{ route("rejected_reports.makeactive", ["report" => "_reportsID_"]) }}'.replace('_reportsID_', reportsID),
                        success: function(response) {
                            // Swal.fire(
